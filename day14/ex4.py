...
class Suning(object):
    ...
    def parse_page(self):
        try:
            skus = self.wait.until(EC.presence_of_all_elements_located((By.XPATH, '//ul[@class="general clearfix"]/li')))
            skus = [item.get_attribute('id').replace('-', '/') for item in skus]
            links = [f'https://product.suning.com/{sku}.html' for sku in skus]

            titles = self.wait.until(EC.presence_of_all_elements_located((By.XPATH, '//ul[@class="general clearfix"]/li//div[@class="title-selling-point"]/a')))
